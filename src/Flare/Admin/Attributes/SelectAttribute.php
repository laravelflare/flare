...
class SelectAttribute extends BaseAttribute
{
    ...
    public function getField()
    {
        ...
        } else if (isset($this->field['options']) && is_string($this->field['options']) && method_exists($this->getModelManager(), $method = camel_case('get_'.$this->field['options'].'_options'))) {
            // Check if Options is a string and if so, check for a method
            // of options based on getDefinedOptions()
            $this->field['options'] = $this->getModelManager()->$method();
        } else if (isset($this->field['options']) && is_string($this->field['options'])) {
